...
function QRmodal({QRcode, setQRcode, orderCode}) {
    ...
    return (
        <div className="QR-modalOverlay">
          <div className="QR-modalContent">
            {orderCode ? (
              <h4>QR Code para el pedido de: {QRcode.cliente} Codigo: {QRcode.orderCode}</h4>) :
    ...
    code: QRCODE,
          ...
          { orderCode ? <QRCodeCanvas value={JSON.stringify({id: QRcode.id, code: QRcode.orderCode})} size={200} /> : <QRCodeCanvas value={JSON.stringify({id: QRcode.id, code: QRcode.productCode})} size={200} /> }
